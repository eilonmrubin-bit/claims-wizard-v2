...
function translateLevelA(pattern, rest_day):
  for each day in range(pattern.start, pattern.end):
    ...
    if dow in pattern.work_days:
      → is_work_day=true, is_rest_day=is_rest
      ...
      → is_work_day=false, is_rest_day=is_rest, shifts=[], breaks=[]
```
...
**הערה:** הפסקה נכתבת ל-daily record רק אם המשתמש הזין אותה במפורש (break_minutes > 0 ב-per_day, או default_breaks לא ריק). אין הזרקה אוטומטית של הפסקה.
...
11. **DO NOT** מזריקים הפסקה אוטומטית. הפסקה נכתבת רק אם המשתמש הזין `break_minutes > 0` או `default_breaks` לא ריק. אם אין קלט הפסקה — אין הפסקה. אין "חובת הפסקה אוטומטית" במערכת.
12. **DO NOT** מסננים את יום המנוחה מ-work_days. אם הדפוס כולל את יום המנוחה — הוא נשאר. שני הדגלים (`is_work_day` ו-`is_rest_day`) יהיו `true`. צינור שע"נ יחיל תעריפי מנוחה.
